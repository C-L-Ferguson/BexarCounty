...
specD_focal <- res |>
  filter(model == "SpecD_ProsecutorCourtFE") |>
  mutate(
...
tex5 <- c(tex5,
  ...
  "\\multicolumn{4}{l}{\\footnotesize largest relative amplification (more than tripling from 2.4 to 8.5 pp); gaps in F2--FS reach 10--12 pp by career peak.} \\\\",
  "\\end{tabular}",
  "\\end{table}"
)
