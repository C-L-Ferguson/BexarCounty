...
tex1 <- c(tex1,
  ...
  "\\multicolumn{5}{l}{\\footnotesize Prosecutor = disposition-stage prosecutor. Career case $N$ = cumulative case count per prosecutor.} \\\\",
...
tex4 <- c(tex4,
  ...
  "\\multicolumn{3}{l}{\\footnotesize split by DA in office when the prosecutor was first observed. Controls: felony} \\\\",
...
# Pooled model: formal Wald test of whether Hispanic prosecutors' learning curve
# differs significantly from White prosecutors'
df_pool <- df_pros |>
  filter(pros_pred_race %in% c("White", "Hispanic")) |>
  mutate(HISP_PROS = as.integer(pros_pred_race == "Hispanic"))

fit_pool <- feglm(
  DEFERRED ~ BLACK + LATINO + OUTTAKE_CASE_N100 +
    BLACK:OUTTAKE_CASE_N100 + LATINO:OUTTAKE_CASE_N100 +
    HISP_PROS +
    HISP_PROS:BLACK + HISP_PROS:LATINO +
    HISP_PROS:OUTTAKE_CASE_N100 +
    HISP_PROS:BLACK:OUTTAKE_CASE_N100 +
    HISP_PROS:LATINO:OUTTAKE_CASE_N100 +
    OFFENSE_TYPE + OFFENSE_CATEGORY2 + APPOINTED,
  data = df_pool,
  fixef = "PROSECUTOR", family = binomial(),
  cluster = "PROSECUTOR")

pool_results <- broom::tidy(fit_pool, conf.int = TRUE) |>
  mutate(model = "Pooled_DiffTest")
...
tex6 <- c(tex6,
  ...
  "\\hline\\hline",
  "\\multicolumn{4}{l}{\\footnotesize \\textit{Notes:} Each column estimates the benchmark model (Table~2) on a subsample defined by predicted prosecutor race.} \\\\",
  ...
  "\\multicolumn{4}{l}{\\footnotesize A Wald test on a pooled model finds Hispanic prosecutors' interaction is significantly larger than White prosecutors' (Black: $p=0.028$; Latino: $p=0.0001$).} \\\\",
...
tex0 <- c(tex0,
  ...
  "\\multicolumn{5}{l}{\\footnotesize from jail booking records (Black, Latino, White). Prior case = defendant had at least one} \\\\",
  "\\multicolumn{5}{l}{\\footnotesize prior case in the Bexar County criminal record before the current case (all available years). Hilbig--Reed} \\\\",
